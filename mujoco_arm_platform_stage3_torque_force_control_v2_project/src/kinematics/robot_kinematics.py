...
class RobotKinematics:
    ...
    def site_jacobian(self, site_name: Optional[str] = None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        return jacp[:, self.qvel_idx], jacr[:, self.qvel_idx]
    ...
    def solve_ik_pose(
        ...
        Warning:
            The arm has 4 controlled DoF, so arbitrary 6D target poses are
            usually overconstrained. Use position IK first unless orientation
            is truly needed.
        """
        target_pos = np.asarray(target_pos, dtype=float).reshape(3)
        ...
        for it in range(max_iter):
            ...
            jacr = np.zeros((3, self.model.nv))
            mujoco.mj_jacSite(self.model, self.data, jacp, jacr, sid)
            J = np.vstack([pos_weight * jacp[:, self.qvel_idx], rot_weight * jacr[:, self.qvel_idx]])
